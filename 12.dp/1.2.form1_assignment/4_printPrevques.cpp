#include <bits/stdc++.h>
using namespace std;
//print a subset that adds upto a q queries of target t;
int n,t,q;
int x[101];
int dp[101][10010];
...
void printer(int level, int sum_left){
    if(level==n+1){
        return ;
    }

    if(rec(level+1,sum_left)==1){//checking next level
        printer(level+1,sum_left);
    }else if(rec(level+1,sum_left-x[level])==1){
        cout<<x[level]<<" ";
        printer(level+1,sum_left-x[level]);
    }
}
