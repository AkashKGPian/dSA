...
vector<vector<vector<state>>> parent,origin,min2;
...
void dijktra(){
    ...
    origin = vector<vector<vector<state>>>(n,vector<vector<state>> (m, vector<state>(2,{{-1,-1}, -1})));
    min2 = vector<vector<vector<state>>>(n,vector<vector<state>> (m, vector<state>(2,{{-1,-1}, -1})));
    
    priority_queue<pair<lli, state>> pq; //dist, state
    for(auto stcell:sc){
        dist[stcell.F][stcell.S][0]=0; // dist(cordn)(0) not seen -2 till now ; dist(cordn)(1) seen -2
        pq.push( MP(-0,MP(stcell,0) ) );
        origin[stcell.F][stcell.S][0]={stcell,0};
    }
        ...
        for(int k=0;k<4;k++){
            ...
            if(isInside(nx,ny)){
                ...
                if( dist[nx][ny][newFlag] > currdist + max(0,arr[nx][ny]) ){
                    ...
                    if(curr.S==0 && newFlag)min2[nx][ny][newFlag]=state({{nx,ny},newFlag});
                    if(curr.S==1)min2[nx][ny][newFlag]=min2[curr.F.F][curr.F.S][curr.S];//imp
                    origin[nx][ny][newFlag]=origin[curr.F.F][curr.F.S][curr.S];
                }
            }
        }
    }
...
int main(){
    ...
    dijktra();// coordinates
    lli ans = 1e9;
    state bestEnPos = {{-1,-1},-1};
    for( auto enpos:en ){
        if(ans > dist[enpos.F][enpos.S][1]){
            bestEnPos = {enpos,1};//at this point -2 is already must be seen
            ans = dist[enpos.F][enpos.S][1];
        }
    }
    cout<<ans<<"\n";
    
    auto temp = origin[bestEnPos.F.F][bestEnPos.F.S][bestEnPos.S];
    cout<<temp.F.F<<", "<<temp.F.S<<"\n";
    temp = min2[bestEnPos.F.F][bestEnPos.F.S][bestEnPos.S];
    cout<<temp.F.F<<", "<<temp.F.S<<"\n";
    cout<<bestEnPos.F.F<<", "<<bestEnPos.F.S;
    

    
    // state curr = MP(bestEnPos,1);
    // vector<ii> Path;
    // while(curr != state({{-1,-1},-1})){
    //     // cout<< curr.F.F <<","<< curr.F.S<<"\n";
    //     path.push_back(MP(curr.F.F,curr.F.S));
